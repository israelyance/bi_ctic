...
select * from TMP_DATA_CLIENTE where periodo = 'periodo' ;
...
select count(*) from H_CLIENTES;

--select  *  from (
select * from H_CLIENTES
where rownum <=10 ;
...
insert into  H_CLIENTES (HCL_PER_COD_SBS,HCL_PERIODO,HCL_COD_SBS_CLIENTE,HCL_FECHA_REPORTE,HCL_TIPO_DOC_EMPRESA,HCL_RUC,HCL_COD_TIPO_DOC_PERSONA,HCL_DOCUMENTO_PERSONA,
...
select *  from ( select  * from H_CLIENTES) 
where rownum <= 2 ;
...
select distinct FECHA_REPORTE from H_CLIENTES;
